...
const GITHUB_API = `https://api.github.com/users/${import.meta.env.VITE_GITHUB_USERNAME || "cyber-rohit-91"}`;
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 mt-7 sm:mt-0 md:mt-3 lg:mt-5">
      <div className="text-center relative z-10 max-w-4xl mx-auto">
        <motion.h3
          className="logo-chakra text-4xl sm:text-6xl md:text-8xl font-bold mb-4 sm:mb-6 relative tracking-tighter"
        ...
        <motion.h6
          className="text-lg sm:text-lg md:text-lg  mb-4 sm:mb-6 relative font-extrabold tracking-wide   tracking-[0.1em] text-cyan-300 drop-shadow-[0_0_0px_rgba(34,211,238,0.5)]"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
        >
          Certified Ethical Hacker (CEH v13) | Web Application Security | Penetration Testing

        </motion.h6>

        <motion.p
          className="text-lg sm:text-xl md:text-xl text-gray-400 mb-4 sm:mb-5 max-w-2xl mx-auto px-2 sm:px-4"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, delay: 0.2 }}
        >          
          Web Application Security enthusiast focused on vulnerability assessment, penetration testing, and ethical hacking practices.
        ...
        >
          <div className="flex justify-center space-x-3 sm:space-x-4">
            ...
            <Link
              to="/about"
              className="px-4 border border-cyan-400/70 sm:px-6 py-2.5 sm:py-3 bg-white/10 text-white rounded-full text-sm sm:text-base font-medium hover:bg-white/20 transition-colors flex items-center gap-2"
            >
          ...
          <button
            ...
            className="group border  border-cyan-400/70 rounded-full relative flex items-center gap-2 py-2 pl-8 pr-4 hover:bg-transparent transition-all cursor-copy sm:cursor-pointer"
            aria-label={`Email: ${CONTACT_INFO.email}`}
          >
            <div className="absolute left-0 flex items-center">
              <div className="w-3 text-yellow-400 group-hover:text-white transition-colors ml-2">
                <VercelLogo />
              </div>
              <span className="text-lg text-yellow-400 font-mono text-gray-400 ml-3 group-hover:text-white transition-colors">
                ~ 
              </span>
            </div>
            <span className="text-yellow-400 group-hover:text-white transition-colors ml-4 sm:text-base">
              {CONTACT_INFO.email}
            </span>
...
<hr className="border-0 h-px bg-white/50 hover:bg-cyan-400/100 transition-all duration-300 my-6" />
